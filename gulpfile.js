...
//config読み込み
const config = require('./config/gulpconfig.json');
...
gulp.task('sass', function(callback) {
	if (setting.mode == 'development') {
		return gulp
			...
			.pipe(gulp.dest(setting.dir.devDir + setting.dir.destCssDir));
...
gulp.task('sass-min', function() {
	if (setting.mode == 'development') {
		...
		return gulp
			...
			.pipe(cleanCSS())
			.pipe(rename({extname: '.min.css'}))
			.pipe(gulp.dest(setting.dir.destDir + setting.dir.destCssDir));
	}
});
...
gulp.task('imagemin', function() {
	if (setting.mode == 'development') {
		return gulp
			...
			.pipe(changed(setting.dir.devDir + setting.dir.destImgDir))
			.pipe(
				imagemin([
			...
			.pipe(gulp.dest(setting.dir.devDir + setting.dir.destImgDir));
	} else if (setting.mode == 'production') {
		return gulp
			.src(setting.dir.imgDir + '**/*.+(jpg|jpeg|png|gif|svg)')
			.pipe(changed(setting.dir.destDir + setting.dir.destImgDir))
			.pipe(
				imagemin([
					pngquant({quality: [0.6, 0.8]}),
					mozjpeg({quality: [80]}),
			...
			.pipe(gulp.dest(setting.dir.destDir + setting.dir.destImgDir));
	}
});
...
gulp.task('webpack', function() {
	if (setting.mode == 'development') {
		...
		return gulp
			...
			.pipe(gulp.dest(setting.dir.devDir + setting.dir.destJsDir));
	} else if (setting.mode == 'production') {
		return gulp
			.src([setting.dir.jsDir + '**/*.js'])
			.pipe(plumber())
			...
			.pipe(eslint.format())
			.pipe(webpackStream(webpackConfig, webpack))
			.pipe(gulp.dest(setting.dir.destDir + setting.dir.destJsDir));
	}
});
...
// dev環境build
gulp.task('dev', gulp.series('sass', 'sass-min', 'css', 'ejs', 'imagemin', 'webpack'));

// リリース用ファイルbuild
gulp.task('build', gulp.series('changeProd', 'clean', 'sass-min', 'css', 'ejs', 'imagemin', 'webpack', 'message'));

// default 各タスク並列実行、serveでwatch
gulp.task('default', gulp.series('sass', 'sass-min', 'css', 'ejs', 'imagemin', 'webpack', 'serve'));
